...
// Versión mejorada: Sistema completo de barcos, colocación, disparos, estadísticas,
// archivos de texto y archivos binarios con acceso aleatorio e indexado.

/*
* ============================================================
*  BATALLA NAVAL (FunFleet) - SPRINT 4
* ============================================================
*  Mantiene TODO lo de los Sprints 1, 2 y 3:
*    - Registro de jugador y menu principal basados en char[].
*    - Colocacion manual de la flota, con validaciones completas.
*    - Carga de la flota desde "fleet-grid.txt" (acceso secuencial).
*    - Exportacion de la flota a "ai-fleet-grid.txt".
*    - Generacion de "funfleet-report.txt".
*
*  Y agrega lo pendiente del Sprint 3 mas lo nuevo del Sprint 4:
*
*    1) FLOTA ALEATORIA: genera los 9 barcos oficiales en posiciones
*       y orientaciones aleatorias, sin salirse del tablero ni
*       superponerse, reintentando hasta lograrlo. Usa srand(time(0))
*       una unica vez en main().
*
*    2) ARCHIVOS BINARIOS CON ACCESO ALEATORIO E INDEXADO:
*       a) "shots.dat": registro de tamaño fijo (RegistroDisparo) por
*          cada disparo valido, escrito con write(). Permite consultar
*          un turno especifico usando seekg() con la formula
*          (turno - 1) * sizeof(RegistroDisparo) y read().
*       b) "funfleet-save.dat": guarda y carga el estado completo de
*          la partida (jugador, tableros, estadisticas y flota) en
*          una unica estructura binaria de tamaño fijo, usando
*          write()/read() y seekp()/seekg().
*
*    3) LIMITE DE TURNOS: la partida ahora tambien termina cuando se
*       agotan los turnos disponibles (constante MAX_TURNOS), ademas
*       de terminar cuando se hunde toda la flota.
*
*  Formato del archivo de texto (Sprint 3, se mantiene igual):
*    10 lineas, cada una con 10 tokens separados por espacios.
*    Tokens validos: ~  P  S  A1  A2  D1  D2  D3  F1  F2
*
*  Compilacion:  g++ sprint4.cpp -o sprint4
*  Ejecucion:    ./sprint4   (los archivos .txt y .dat se leen/escriben
...
// ============================================================
//   No se usan prototipos: todas las funciones se implementan
//   en orden de dependencias, de abajo hacia arriba, y main()
//   queda siempre al final del archivo.
// ============================================================
		...
		if (strcmp(comandoMayus, "DISPARAR") == 0) {
			...
			if (procesarDisparo(posX, posY, resultado, codigoBarcoImpactado)) {
				...
				if (todosBarcosHundidos()) {
					...
					cout << "Turno " << disparosRealizados << " de " << MAX_TURNOS << "." << endl;
				}
			}
...
int main() {
	...
	do {
		...
		opcion = leerOpcionValida(1, 10, "Seleccione una opcion: ");
